...
def gaussian_kernel(radius, sigma=2., order=0):
...
def pad_symmetric(tensor, padding):
    pad_left, pad_right = padding
    in_sizes = tensor.size()
...
def peak(tensor, edge=float('-inf')):  # HxW
...
def extract_poses(all_peaks, paf_avg, height, thre2):
...
class Body(object):
    ...
    def __call__(self, oriImg):
        ...
        for m in range(len(multiplier)):
            ...
            heatmap = np.transpose(np.squeeze(Mconv7_stage6_L2), (1, 2, 0))  # output 1 is heatmaps -channel last
        ...
        for part in range(18):
            ...
            peaks_with_score_and_id = [peaks_with_score[i] + (peak_id[i],) for i in range(len(peak_id))]  # (X,Y,conf,id)

            all_peaks.append(peaks_with_score_and_id)
            peak_counter += len(peaks)

        return extract_poses(all_peaks, paf_avg, oriImg.shape[0], thre2)
